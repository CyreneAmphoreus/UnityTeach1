...
基础练习中直接使用 `WorldToScreenPoint` / `ScreenToWorldPoint` 很合适。实际项目中，如果要根据鼠标点击在场景中放置物体，更常见的是使用 `Camera.ScreenPointToRay` 发射射线，再与地面、Plane 或 Collider 求交点。

### 旧写法和新写法的区别

- 旧输入系统可用 `Input.GetMouseButtonDown(0)` 和 `Input.mousePosition`。
- 新 Input System 可以通过 Action 绑定 `<Mouse>/leftButton`，在 `performed` 回调里处理点击。
- `Input.mousePosition` 在当前项目中仍可读取鼠标屏幕坐标，但输入触发由新 Input System 管理。

### WorldToScreenPoint

`WorldToScreenPoint` 用于把世界坐标转换为屏幕坐标：

```csharp
Vector3 pos = Camera.main.WorldToScreenPoint(transform.position);
```

返回值中的 `x`、`y` 是屏幕像素坐标，`z` 表示该点距离摄像机的深度。

### ScreenToWorldPoint

`ScreenToWorldPoint` 用于把屏幕坐标转换为世界坐标。使用时必须给 `z` 一个有意义的深度：
...
这里的 `z = 10` 表示距离摄像机 10 个单位的深度平面。它不是屏幕二维坐标的一部分。

### 适合初学阶段的选择建议

初学阶段先用固定 `z` 理解屏幕坐标转世界坐标即可。等理解了 Camera 视锥和深度概念后，再学习射线检测，会更自然。
